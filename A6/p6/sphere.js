...
float xztorusSDF(vec3 samplePoint, float R, float r) {
  ...
  samplePoint.y =samplePoint.y +0.2;
  float x = length(samplePoint.xy)- R;
  return length(vec2(x, samplePoint.z)) -r;
}
float sceneSDF(vec3 samplePoint) {
  ...
  float t4 = xztorusSDF((samplePoint.xyz +vec3(0.35, 0, 0.)), 0.3, 0.05);     //draw yellow
  float t5 = xztorusSDF((samplePoint.xyz + vec3(-0.35, 0, 0.)), 0.3, 0.05);   //draw green
  float m1 = min(t1,t2);
  float m2 = min(t3,t4);
  float m3= min(t4,t5);
  float n1= min(m1,m2);
