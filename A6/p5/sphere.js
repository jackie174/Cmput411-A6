...
float sceneSDF(vec3 samplePoint) {
  ...
  float t1 = xytorusSDF((samplePoint.xyz + vec3(0.0, 0., 0.)), 0.3, 0.05);
  float t2 = xytorusSDF((samplePoint.xyz + vec3(0.7, 0., 0.)), 0.3, 0.05);
  float t3 = xytorusSDF((samplePoint.xyz + vec3(-0.7, 0., 0.)), 0.3, 0.05);
  float t4 = xytorusSDF((samplePoint.xyz + vec3(0.35, 0.2, 0.)), 0.3, 0.05);
  float t5 = xytorusSDF((samplePoint.xyz + vec3(-0.35, 0.2, 0.)), 0.3, 0.05);
  float m1 = min(t1,t2);
  float m2 = min(t3,t4);
  float m3= min(t4,t5);
  float n1= min(m1,m2);
...
void main(void )
{
    ...
    vec3 p = eye + dist * dir;
    //---------------------------------------black-------------------------------------------
    dir = rayDirection(45.0, fragCoord.xy);
    eye = vec3(0.0, 0.0, 5.0);
    dist = shortestDistanceToSurface(eye, dir, MIN_DIST, MAX_DIST);
    p = eye + dist * dir;
    if(c == 1.0){
